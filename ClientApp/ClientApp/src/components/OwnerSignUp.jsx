    ...
    axios.post(url, formData)
      .then(response => {
        if (response.data.status) {
          ...
          // Here, send an email using EmailJS
          const templateParams = {
            to_name: formData.ownerName,
            to_email: formData.email,
            message: `Congratulations you are now a registered Owner. Your id is ${ formData.owner_id}. Your login id is ${ formData.email} and your password is ${ formData.password}`,
          };
          
          // Replace 'YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', and 'YOUR_USER_ID' with your actual EmailJS values
          emailjs.send('service_vd4bfmk', 'template_xb7qzql', templateParams, 'DqVjEjSRmJsYkTzK-')
            .then((response) => {
          ...
          // Optionally navigate to sign-in page or show success message
          setIsSubmitted(true); // If you want to display a success message
          // navigate("/signin");
        } else {
